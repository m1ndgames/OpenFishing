...
<div style="max-width:520px; margin:0 auto;">
	...
	<form method="POST" style="background:var(--of-bg-surface); border:1px solid var(--of-border-subtle); border-radius:16px; padding:24px 28px; display:flex; flex-direction:column; gap:20px;">
		<h1 style="font-family:'Carter One',sans-serif; font-size:1.4rem; color:var(--of-text-bright); margin:0;">{t.lineAddTitle}</h1>
		<div>
		...
		<div>
			...
			<select name="type" style="{inputStyle} appearance:none;">
				<option value="">—</option>
				<option value="Braid">{t.lineTypeBraid}</option>
				<option value="Mono">{t.lineTypeMono}</option>
				<option value="Fluoro">{t.lineTypeFluoro}</option>
			</select>
		</div>
		<div class="of-grid-2-sm">
			<div>
				<label style={labelStyle}>{t.lineDiameter}</label>
				<input name="diameter_mm" type="number" step="0.01" min="0" style={inputStyle} onfocus={focusInput} onblur={blurInput} />
			</div>
			<div>
				<label style={labelStyle}>{t.lineStrength}</label>
				<input name="strength_kg" type="number" step="0.1" min="0" style={inputStyle} onfocus={focusInput} onblur={blurInput} />
			</div>
		</div>
		<div>
