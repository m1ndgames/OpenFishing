...
<div style="max-width:520px; margin:0 auto;">
	...
	<form method="POST" style="background:var(--of-bg-surface); border:1px solid var(--of-border-subtle); border-radius:16px; padding:24px 28px; display:flex; flex-direction:column; gap:20px;">
		<h1 style="font-family:'Carter One',sans-serif; font-size:1.4rem; color:var(--of-text-bright); margin:0;">{t.comboAddTitle}</h1>
		<div>
		...
		<div>
			...
			{#if rods.length === 0}
				<p style="font-size:0.72rem; color:var(--of-text-4); margin:4px 0 0;"><a href="/tackle/rods/new" style="color:var(--of-accent);">{t.rodAddTitle}</a></p>
			{/if}
		</div>
		<div>
			...
			{#if reels.length === 0}
				<p style="font-size:0.72rem; color:var(--of-text-4); margin:4px 0 0;"><a href="/tackle/reels/new" style="color:var(--of-accent);">{t.reelAddTitle}</a></p>
			{/if}
		</div>
		<div>
