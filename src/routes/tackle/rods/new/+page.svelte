...
<div style="max-width:520px; margin:0 auto;">
	...
	<form method="POST" style="background:var(--of-bg-surface); border:1px solid var(--of-border-subtle); border-radius:16px; padding:24px 28px; display:flex; flex-direction:column; gap:20px;">
		<h1 style="font-family:'Carter One',sans-serif; font-size:1.4rem; color:var(--of-text-bright); margin:0;">{t.rodAddTitle}</h1>
		<div>
		...
		<div class="of-grid-2-sm">
			<div>
				...
				<input name="length_m" type="number" step="0.01" min="0" style={inputStyle} onfocus={focusInput} onblur={blurInput} />
			</div>
			<div>
				<label style={labelStyle}>{t.rodCastingWeight}</label>
				<input name="casting_weight" type="text" placeholder="e.g. 7-28" style={inputStyle} onfocus={focusInput} onblur={blurInput} />
			</div>
		</div>
		<div>
			...
			<select name="type" style="{inputStyle} appearance:none;">
				<option value="">—</option>
				<option value="Spinning">{t.rodTypeSpinning}</option>
				<option value="Casting">{t.rodTypeCasting}</option>
			</select>
		</div>
		<div>
