...
export const actions: Actions = {
	// Regular users edit their own email + username. The admin's identity is fully
	// env-controlled (fixed username "admin", no email), so the admin cannot change it.
	updateProfile: async ({ request, locals }) => {
		if (!locals.user) redirect(303, '/');
		if (locals.user.isAdmin) return fail(403, { error: 'cannotModifyAdmin' });
	...
	// The admin's password comes from the environment and is re-synced on every startup,
	// so it cannot be changed here.
	changePassword: async ({ request, locals, cookies }) => {
		if (!locals.user) redirect(303, '/');
		if (locals.user.isAdmin) return fail(403, { error: 'adminPasswordEnvControlled' });
