...
export const load: PageServerLoad = async () => {
	...
	// Global default appearance (the login-screen look + fallback for users with no preference).
	const defaults = await db.select().from(appSetting).where(inArray(appSetting.key, ['colorMode', 'themeName']));
	const dm = Object.fromEntries(defaults.map((s) => [s.key, s.value]));

	return {
		users,
		chatbotConfigured: !!env.CHATBOT,
		defaultColorMode: (dm.colorMode ?? 'dark') as 'dark' | 'light' | 'system',
		defaultThemeName: dm.themeName ?? 'ocean'
	};
};

async function setGlobalDefault(key: string, value: string) {
	await db
		.insert(appSetting)
		.values({ key, value })
...
export const actions: Actions = {
	// Set the instance-wide default color mode / theme (login screen + per-user fallback).
	setDefaultMode: async ({ request }) => {
		const mode = ((await request.formData()).get('colorMode') as string) ?? '';
		if (!['dark', 'light', 'system'].includes(mode)) return fail(400, { error: 'invalidValue' });
		await setGlobalDefault('colorMode', mode);
		return { success: 'defaultsSaved' };
	},

	setDefaultTheme: async ({ request }) => {
		const theme = ((await request.formData()).get('themeName') as string) ?? '';
		if (!THEME_IDS.includes(theme)) return fail(400, { error: 'invalidValue' });
		await setGlobalDefault('themeName', theme);
		return { success: 'defaultsSaved' };
	},
	...
	updateUser: async ({ request }) => {
		...
		// The admin's identity (username "admin", no email) and password are env-controlled —
		// only its quota is editable here. Regular users: email/username/quota/password.
		if (target.isAdmin) {
			await db.update(user).set({ quotaBytes, updatedAt: new Date() }).where(eq(user.id, id));
		...
		await db.update(user).set({ email, username, quotaBytes, updatedAt: new Date() }).where(eq(user.id, id));
