...
<div style="max-width:680px;">
	...
	<!-- Photo slider -->
	{#if hasPhotos}
		<div style="position:relative; border-radius:14px; overflow:hidden; background:var(--of-bg-overlay); aspect-ratio:16/9; margin-bottom:16px;">
	...
	<div style="background:var(--of-bg-surface); border:1px solid var(--of-border-subtle); border-radius:14px; padding:20px; display:flex; flex-direction:column; gap:16px;">
		...
		<div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap;">
			<div>
				<h1 style="font-family:'Carter One',sans-serif; font-size:1.4rem; color:var(--of-text-bright); margin:0 0 4px;">{spot.name}</h1>
				<p style="font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--of-text-4); margin:0;">{formatCoord(spot.lat)}, {formatCoord(spot.lng)}</p>
			</div>
		...
		{#if spot.tags.length > 0}
			<div>
				<p style="font-size:0.72rem; font-weight:500; color:var(--of-text-4); text-transform:uppercase; letter-spacing:0.06em; margin:0 0 8px;">{t.spotTagsLabel}</p>
				<div style="display:flex; flex-wrap:wrap; gap:6px;">
					{#each spot.tags as tag}
						<span style="font-size:0.78rem; font-weight:600; color:var(--of-text-3); background:rgba(93,143,168,0.1); border:1px solid rgba(93,143,168,0.2); padding:3px 10px; border-radius:20px;">{tag.name}</span>
					{/each}
				</div>
			</div>
		{/if}
		...
		{#if spot.notes}
			<div>
				<p style="font-size:0.72rem; font-weight:500; color:var(--of-text-4); text-transform:uppercase; letter-spacing:0.06em; margin:0 0 6px;">{t.spotNotesLabel}</p>
				<p style="font-size:0.875rem; color:var(--of-text-2); margin:0; line-height:1.6; white-space:pre-wrap;">{spot.notes}</p>
			</div>
		{/if}
	...
	<!-- Weather card -->
	{#if weather}
		<div style="background:var(--of-bg-surface); border:1px solid var(--of-border-subtle); border-radius:14px; padding:16px 20px; margin-top:16px;">
			<p style="font-size:0.72rem; font-weight:500; color:var(--of-text-4); text-transform:uppercase; letter-spacing:0.06em; margin:0 0 12px;">{t.weatherTitle}</p>

			<!-- Bite index banner -->
			<div style="background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); border-radius:10px; padding:14px 18px; margin-bottom:12px; display:flex; align-items:center; gap:16px;">
				<div style="flex:1; min-width:0;">
					...
					>
						<span style="font-size:0.7rem; font-weight:600; color:var(--of-text-4); text-transform:uppercase; letter-spacing:0.06em;">{t.biteIndex}</span>
						<svg width="13" height="13" viewBox="0 0 24 24" fill="none" style="color:var(--of-border-strong); flex-shrink:0;">
				...
				<div style="text-align:right; flex-shrink:0;">
					...
					<span style="font-size:0.8rem; color:var(--of-text-4);">/10</span>
				</div>
			</div>
			...
			<div style="display:grid; grid-template-columns:repeat(4,1fr); gap:8px;">
				...
				<!-- Temperature -->
				<div style="background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); border-radius:10px; padding:12px 10px; display:flex; flex-direction:column; align-items:center; gap:6px; text-align:center;">
					<svg width="20" height="20" viewBox="0 0 24 24" fill="none" style="color:var(--of-accent); flex-shrink:0;">
					...
					<span style="font-family:'JetBrains Mono',monospace; font-size:1rem; font-weight:700; color:var(--of-text-bright); line-height:1;">{weather.temperature != null ? `${weather.temperature}°` : '—'}</span>
					<span style="font-size:0.65rem; color:var(--of-text-4); font-weight:500; text-transform:uppercase; letter-spacing:0.05em;">{t.weatherTemp}</span>
				</div>
				...
				<div style="background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); border-radius:10px; padding:12px 10px; display:flex; flex-direction:column; align-items:center; gap:6px; text-align:center;">
					...
					<span style="font-family:'JetBrains Mono',monospace; font-size:1rem; font-weight:700; color:var(--of-text-bright); line-height:1;">{weather.humidity != null ? `${weather.humidity}%` : '—'}</span>
					<span style="font-size:0.65rem; color:var(--of-text-4); font-weight:500; text-transform:uppercase; letter-spacing:0.05em;">{t.weatherHumidity}</span>
				</div>
				...
				<div style="background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); border-radius:10px; padding:12px 10px; display:flex; flex-direction:column; align-items:center; gap:6px; text-align:center;">
					...
					<span style="font-family:'JetBrains Mono',monospace; font-size:0.85rem; font-weight:700; color:var(--of-text-bright); line-height:1;">{weather.pressure != null ? `${weather.pressure}` : '—'}<span style="font-size:0.6rem; color:var(--of-text-3);"> hPa</span></span>
				<span style="font-size:0.65rem; color:{weather.pressureDelta < -1.5 ? 'var(--of-accent)' : weather.pressureDelta > 1.5 ? 'var(--of-warning-solid)' : 'var(--of-text-4)'};">
					{weather.pressureDelta < -1.5 ? '↓' : weather.pressureDelta > 1.5 ? '↑' : '→'} {weather.pressureDelta > 0 ? '+' : ''}{weather.pressureDelta} hPa
				</span>
					<span style="font-size:0.65rem; color:var(--of-text-4); font-weight:500; text-transform:uppercase; letter-spacing:0.05em;">{t.weatherPressure}</span>
				</div>

				<!-- Moon phase -->
				<div style="background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); border-radius:10px; padding:12px 10px; display:flex; flex-direction:column; align-items:center; gap:6px; text-align:center;">
					<span style="font-size:1.25rem; line-height:1;">{moonEmojis[weather.moonPhaseIndex]}</span>
					<span style="font-size:0.75rem; font-weight:600; color:var(--of-text); line-height:1.2;">{moonPhaseLabels[weather.moonPhaseIndex]}</span>
					<span style="font-size:0.65rem; color:var(--of-text-4); font-weight:500; text-transform:uppercase; letter-spacing:0.05em;">{t.weatherMoon}</span>
				</div>
	...
	{#if showBiteInfo && weather}
		...
		>
			<div
				...
				style="background:var(--of-bg-surface); border:1px solid var(--of-border); border-radius:16px; padding:24px; max-width:420px; width:100%;"
			>
				<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
					<h2 style="font-family:'Carter One',sans-serif; font-size:1rem; color:var(--of-text-bright); margin:0;">{t.biteInfoTitle}</h2>
					<button onclick={() => showBiteInfo = false} style="background:none; border:none; cursor:pointer; color:var(--of-text-4); padding:4px;">
				...
				<div style="margin-bottom:14px;">
					<div style="display:flex; align-items:baseline; justify-content:space-between; margin-bottom:4px;">
						<div style="display:flex; align-items:center; gap:8px;">
							<span style="font-size:0.82rem; font-weight:600; color:var(--of-text);">{t.biteInfoPressure}</span>
							<span style="font-size:0.65rem; color:var(--of-text-4); background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); padding:1px 6px; border-radius:10px;">50% {t.biteInfoWeight}</span>
						</div>
					...
					<div style="background:var(--of-border-subtle); border-radius:3px; height:4px; margin-bottom:5px;">
						<div style="background:{biteColor(ps)}; border-radius:3px; height:4px; width:{ps*10}%;"></div>
					</div>
					<p style="font-size:0.72rem; color:var(--of-text-4); margin:0;">
				...
				<div style="margin-bottom:14px;">
					<div style="display:flex; align-items:baseline; justify-content:space-between; margin-bottom:4px;">
						<div style="display:flex; align-items:center; gap:8px;">
							<span style="font-size:0.82rem; font-weight:600; color:var(--of-text);">{t.biteInfoLight}</span>
							<span style="font-size:0.65rem; color:var(--of-text-4); background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); padding:1px 6px; border-radius:10px;">30% {t.biteInfoWeight}</span>
						</div>
					...
					<p style="font-size:0.72rem; color:var(--of-text-4); margin:0;">{t.biteInfoLightDesc}</p>
				</div>
				...
				<div style="margin-bottom:20px;">
					<div style="display:flex; align-items:baseline; justify-content:space-between; margin-bottom:4px;">
						<div style="display:flex; align-items:center; gap:8px;">
							<span style="font-size:0.82rem; font-weight:600; color:var(--of-text);">{t.biteInfoTemp}</span>
							<span style="font-size:0.65rem; color:var(--of-text-4); background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); padding:1px 6px; border-radius:10px;">20% {t.biteInfoWeight}</span>
						</div>
				...
				<!-- Formula result -->
				<div style="background:var(--of-bg-overlay); border:1px solid var(--of-border-subtle); border-radius:10px; padding:12px 16px; font-family:'JetBrains Mono',monospace; font-size:0.78rem; color:var(--of-text-3);">
				...
				<table style="width:100%; border-collapse:collapse; font-size:0.82rem;">
					<thead>
						<tr style="border-bottom:1px solid var(--of-border-subtle);">
							...
							<th style="text-align:left; font-size:0.68rem; font-weight:500; color:var(--of-text-4); text-transform:uppercase; letter-spacing:0.06em; padding:0 0 8px 12px;">{t.catchLureLabel}</th>
						</tr>
					</thead>
					<tbody>
						{#each nearbyCatches as c}
							...
							>
								...
								<td style="padding:8px 10px 8px 0; width:40px;">
									...
									{:else}
										<div style="width:36px; height:28px; border-radius:5px; background:var(--of-bg-overlay); display:flex; align-items:center; justify-content:center; flex-shrink:0;">
											<svg width="14" height="14" viewBox="0 0 24 24" fill="none" style="color:var(--of-border-subtle);">
								...
								<td style="padding:8px 0 8px 12px; color:var(--of-text-3); white-space:nowrap; max-width:120px; overflow:hidden; text-overflow:ellipsis;">
